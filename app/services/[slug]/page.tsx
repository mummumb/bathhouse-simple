...
import { getManagedContentBySectionName } from "@/lib/data-utils"
import Image from "next/image"

export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const service = services.find((s) => s.slug === slug)
  const sectionName = `service-${slug}`
  const contentSections = await getManagedContentBySectionName(sectionName)

  if (!service || !contentSections || contentSections.length === 0) {
    notFound()
  }

  const mainContent = contentSections[0]
        ...
        <div className="bg-bathhouse-cream">
          <div className="bathhouse-container grid lg:grid-cols-3 gap-12 lg:gap-16 py-16 sm:py-20">
            ...
            <div className="lg:col-span-2">
              ...
              <section id="page-content" className="mt-12">
                <div className="container mx-auto px-4 py-8">
                  <article className="prose prose-lg mx-auto max-w-3xl">
                    {mainContent.image_url && (
                      <Image
                        alt={mainContent.image_alt || mainContent.title}
                        className="mb-8 h-64 w-full rounded-lg object-cover"
                        height={400}
                        src={mainContent.image_url || "/placeholder.svg"}
                        width={800}
                      />
                    )}
                    <h1 className="mb-4 text-4xl font-bold">{mainContent.title}</h1>
                    <div className="service-content prose prose-lg max-w-none" dangerouslySetInnerHTML={{ __html: mainContent.content }} />
                  </article>
                </div>
              </section>
            </div>
